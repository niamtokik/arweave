...
move_chunk_storage(SrcDir, DstDir) ->
  ...
  % ar:console(Mv),
  % os:cmd(Mv).
  {ok, Files} = file:list_dir(ChunkStorageDir),
  Mv = fun(File) ->
               ChunkFileSrc = file:join([ChunkStorageDir, File]),
               ChunkFileDst = file:join([DstDir, File]),
               ar:console("move ~s to ~s", [ChunkFileSrc, ChunkFileDst]),
               ok = file:rename(ChunkFileSrc, ChunkFileDst)
       end,
  _ = lists:map(Mv).
